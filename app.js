...
function render(){
  ...
  byId('standingsBody').innerHTML = table.map((t,i)=>`<tr><td class="rank">${i+1}</td><td class="team"><img src="${t.logo}" alt="" style="width:54px;height:54px;object-fit:contain;vertical-align:middle;margin-right:12px">${t.name}</td><td>${t.g}</td><td>${t.w}</td><td>${t.d}</td><td>${t.l}</td><td>${t.gf-t.ga > 0 ? '+' : ''}${t.gf-t.ga}</td><td class="points">${t.p}</td></tr>`).join('');
...
async function loadRemoteData(){
  ...
  const { data: remoteMatches, error } = await supabaseClient.from('matches').select('*').order('id');
...
async function init(){
  if (supabaseClient) { const {data:{session}}=await supabaseClient.auth.getSession(); loggedIn=Boolean(session); supabaseClient.channel('partite-aggiornate').on('postgres_changes',{event:'*',schema:'public',table:'matches'},loadRemoteData).subscribe(); }
  loadRemoteData();
}
init();
