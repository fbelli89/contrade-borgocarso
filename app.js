...
function render(selectedMatchId = byId('matchSelect').value){
  ...
  byId('goalSelect').innerHTML=data.goals.map(goal=>{const match=data.matches.find(item=>item.id===goal.match_id);return `<option value="${goal.id}">${goal.player} · ${team(goal.team).name}${match?` · ${match.day}`:''}</option>`}).join('') || '<option value="">Nessun marcatore registrato</option>';
