const defaultData = {
  teams: [
    { id: 'nord', name: 'Nord', color: '#277a93', logo: 'assets/loghi/nord.png' },
    { id: 'sud', name: 'Sud', color: '#a6373e', logo: 'assets/loghi/sud.png' },
    { id: 'centro-storico', name: 'Centro Storico', color: '#557a48', logo: 'assets/loghi/centro-storico.png' },
...
const matchDetails = index => ({
  day: index === 6 ? 'Finale' : `Partita ${index + 1}`,
  date: `Domenica 13 settembre 2026 · ore ${matchSchedule[index]}`
});
...
function render(selectedMatchId = byId('matchSelect').value){
  ...
  byId('matchSelect').innerHTML = data.matches.map((m,index)=>`<option value="${m.id}">${matchDetails(index).day} · ${team(m.home).name} – ${team(m.away).name}</option>`).join('') || '<option value="">Nessuna partita in calendario</option>';
...
byId('calendarForm').onsubmit=async e=>{e.preventDefault();const home=byId('homeTeam').value,away=byId('awayTeam').value,details=matchDetails(data.matches.length);if(!details || data.matches.length >= matchSchedule.length)return toast('Il calendario prevede sei partite e una finale.');if(home===away)return toast('Scegli due contrade diverse.');if(data.matches.some(m=>(m.home===home&&m.away===away)||(m.home===away&&m.away===home)))return toast('Questa partita è già in calendario.');const match={day:details.day,date:details.date,home,away,home_score:null,away_score:null};const {data: inserted,error}=await supabaseClient.from('matches').insert(match).select().single();if(error)return toast('Impossibile aggiungere la partita.');data.matches.push({id:inserted.id,day:inserted.day,date:inserted.date,home:inserted.home,away:inserted.away,homeScore:null,awayScore:null});render(inserted.id);toast('Partita aggiunta al calendario.');};
